    ...
    for (let y = 0; y < H; y += 2) {
        for (let x = 0; x < W; x += 2) {
            ...
            if (imageData[index] > 128) {
                ...
                const nx = (x / W - 0.5) * 2;
                const ny = -(y / H - 0.5) * 2; // Invert Y
                validPixels.push({ x: nx, y: ny });
            }
        }
    }

    if (validPixels.length === 0) return generateSphere(count, size); // Fallback

    for (let i = 0; i < count; i++) {
        const pixel = validPixels[Math.floor(Math.random() * validPixels.length)];
        // Scale to shape size
        const x = pixel.x * size * 1.5; // Wider
        const y = pixel.y * size * 0.6;
        const z = (Math.random() - 0.5) * size * 0.2; // Thin depth
